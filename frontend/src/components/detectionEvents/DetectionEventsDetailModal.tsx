...
import {
  ...
  CheckCircle, Eye, XCircle, MessageSquare, Download, Share2, RefreshCcw
} from 'lucide-react';
...
export function DetectionEventsDetailModal({ event, onClose, onStatusChange, onAddNote, canResolve }: EventDetailModalProps) {
  ...
  // 💡 [버그 픽스] 크래시 방지를 위해 API 문자열이든 Date 객체든 무조건 안전한 Date 객체로 변환
  const safeDate = new Date(event.timestamp);
  ...
  return (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white flex flex-col">
          {/* 💡 [버그 픽스] 스크롤이 내용 안에서만 돌게 하기 위해 shrink-0 추가 */}
          <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between z-10 shrink-0">
            <div>
          ...
          {/* 💡 [버그 픽스] flex-1과 overflow-y-auto 추가하여 내부 스크롤 강제 */}
          <div className="p-6 space-y-6 flex-1 overflow-y-auto min-h-0">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-green-700">
                        ...
                        {event.resolvedAt && ` • ${new Date(event.resolvedAt).toLocaleString('ko-KR')}`}
                      </p>
                    </div>
                  </div>
                </Card>
            ...
            {canResolve && (
                <div className="space-y-3 pt-2">
                  {/* 💡 [기능 추가] 처리 완료 상태일 때 다시 되돌릴 수 있는 버튼 노출 */}
                  {event.status === 'resolved' ? (
                      <Button
                          variant="outline"
