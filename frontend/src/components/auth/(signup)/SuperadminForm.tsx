...
type SuperadminFormProps = {
  ...
  onChange: (
    key: 'name' | 'loginId' | 'password' | 'confirmPassword' | 'email' | 'phone' | 'department',
    value: string
  ) => void;
...
export function SuperadminForm({ form, onChange, fieldErrors }: SuperadminFormProps) {
  ...
  return (
    <section className="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
      <h2 className="text-sm font-semibold text-slate-700">행정청(슈퍼관리자) 정보</h2>
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-slate-700">행정청 부서명</label>
        <input
          type="text"
          name="department"
