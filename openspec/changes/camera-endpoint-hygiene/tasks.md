...
## Lane A — backend
- [ ] A1 `CameraStreamController` GET list：`kindergartenId` 改 `@RequestParam(required=false)`（service 仍以 ThreadLocal `activeKindergartenId` 为准）
- [ ] A2 `CctvCameraController` GET list：同上
- [ ] A3 `EnumMetadataService.REGISTRY` 增 `"camera_stream_type" → CameraStreamTypeEnum.class`、`"protocol" → ProtocolEnum.class`
- [ ] A4 测试：`GET /api/v1/enums/camera_stream_type`、`/protocol` 返回正确码序（仿现有 enums 端点测试）；两 GET 端点不传 kindergartenId 仍返回本租户数据
- [ ] A5 `gradlew test` 全绿
