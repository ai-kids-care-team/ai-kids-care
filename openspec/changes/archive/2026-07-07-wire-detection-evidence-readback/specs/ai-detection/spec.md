...
Legacy or not-yet-uploaded rows whose `storage_uri` is a local `file://` path are reported as
unavailable (not an error), since the AI-side upload to the object store is a separate real-stream
evolution.
...
#### Scenario: Legacy file:// evidence is reported unavailable
...
- **THEN** the evidence is reported as `available = false` (metadata still listed) and its content
  endpoint returns 404, rather than raising an error
