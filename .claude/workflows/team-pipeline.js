export const meta = {
  name: 'team-pipeline',
  description: 'Discovery(人工门外置)→ 并行细化规划 → 按 work-item 并发 implement<->review(worktree + 语义分支 <prefix>/<slug>,默认 task/)→ 集成评审。各 item 独立成 lane 并发推进;高风险整批在规划后 halt 待人工批准(approved=true 续跑)。合并入 develop 仍由 Lead 人工执行。',
  phases: [
    { title: 'Plan' },
    { title: 'Implement' },
    { title: 'Review' },
...
// 语义分支前缀:不用 codex/(我们用 Claude Code,非 Codex);分支名 = <prefix>/<语义 slug id>
const BRANCH_PREFIX = (a && a.branchPrefix) ? a.branchPrefix : 'task'
...
if (preItems) {
  ...
  const decomp = await agent(
    ...
    `【id 命名】每个 item 的 id 用简短【语义化 kebab-case slug】描述其内容(如 announcement-dto-cleanup、delete-empty-controllers),不要用 LANE-1 / TASK-01 之类无语义编号——它会直接成为工作分支名 ${BRANCH_PREFIX}/<id>。\n` +
...
const plans = (await parallel(skeleton.map((it) => () =>
  agent(
    ...
    `【CI 对齐(重要)】若本 item 改变了某命令的【调用方式或必需输入】(例如新增必填环境变量、改命令签名/默认值),必须把所有【调用该命令的地方】(.github/workflows/* 的 CI job、scripts/*)一并纳入 allowedFiles 同步更新;且 verify 命令要用 CI 实际执行的【确切命令】,不要额外导出变量或加旗标来掩盖 CI 会遇到的失败。\n` +
...
// ---- Implement <-> Review:每个 item 一条并发 lane(worktree + 语义分支 <BRANCH_PREFIX>/<id>) ----
const results = await pipeline(plans, async (p) => {
  ...
  for (let round = 1; round <= MAX_FIX; round++) {
    impl = await agent(
      ...
      `分支策略:先 git worktree prune(清理已回收 worktree 的占用)。若分支 ${BRANCH_PREFIX}/${p.id} 已存在(修复轮)→ git checkout ${BRANCH_PREFIX}/${p.id} 继续(失败则 git checkout -B ${BRANCH_PREFIX}/${p.id} ${BRANCH_PREFIX}/${p.id});否则 git checkout -b ${BRANCH_PREFIX}/${p.id} develop。\n` +
      `只改这些文件:${p.allowedFiles.join(', ')}。计划:${p.plan}\n验收点:${(p.acceptance || []).join(' | ')}\n` +
      `本地验证(用仓库命令):${(p.verify || []).join(' ; ')}。验证通过后必须 git add + git commit 到 ${BRANCH_PREFIX}/${p.id}(这是唯一交付载体——worktree 会被回收,未 commit 的改动会丢失)。不要 push,不要切到/改 develop。保留 Korean 文案与代码标识符。\n` +
    ...
    verdict = await agent(
      ...
      `审读分支 diff:git --no-pager diff develop...${BRANCH_PREFIX}/${p.id}。对照计划与验收点评审,跑受影响验证(${(p.verify || []).join(' ; ')})与 git diff --check。\n` +
      ...
      `③ CI 对齐:若本 item 改动影响某命令的调用方式(如新增必填环境变量),按 CI 实际执行的【确切命令】(不额外导出变量)验证;并确认相关 .github/workflows / scripts 已同步更新,否则判 fail。\n` +
      `计划:${p.plan}\n验收点:${(p.acceptance || []).join(' | ')}\n实现摘要:${impl}`,
    ...
    if (verdict.pass || blocking.length === 0) {
      ...
      return { id: p.id, title: p.title, branch: `${BRANCH_PREFIX}/${p.id}`, pass: true, rounds: round, verdict, nonBlocking: p3 }
    }
    log(`${p.id} round ${round}: ${blocking.length} 条阻断(P0-P2)— 循环修复。`)
  }
  log(`✗ ${p.id} 用尽 ${MAX_FIX} 轮仍未过 — 交人工。`)
  return { id: p.id, title: p.title, branch: `${BRANCH_PREFIX}/${p.id}`, pass: false, rounds: MAX_FIX, exhausted: true, verdict }
})
