...
| FE-2 | 前端不再传服务端身份字段（感谢信流：senderUserId 等由服务端派生） | **阻塞 → 已决策（2026-06-19）**：经核实后端**无 `AppreciationLetterController`**（感谢信 read/write 端点均不存在；契约测试确认 `AppreciationLetterService` create/update 已移除）。维护者已决定**新建感谢信后端端点**（见新增 BE-5），FE-2 待 BE-5 就绪后进行。 | P2（阻塞于 BE-5） |
| BE-5 | 新建感谢信（appreciation letters）后端端点（read/write，按 SPEC-0001 租户/角色授权、敏感字段最小化、服务端派生 sender 身份）——维护者 2026-06-19 决策新建，稍后安排。FE-2 依赖之。 | 维护者排期 | P2（待排期） |
...
| CI-4 | 前端测试 + AI 测试纳入 CI | 依赖 FE/AI 测试先行编写 | P3（AI 测试基线已修复，见下注） |
