...
| FE-2 | 前端不再传服务端身份字段（感谢信流：senderUserId 等由服务端派生） | **阻塞 → 已决策（2026-06-19）**：经核实后端**无 `AppreciationLetterController`**（感谢信 read/write 端点均不存在；契约测试确认 `AppreciationLetterService` create/update 已移除）。维护者已决定**新建感谢信后端端点**（见新增 BE-5），FE-2 待 BE-5 就绪后进行。**BE-5 已 Done（2026-06-20），FE-2 解除阻塞、现可进行**：按 SPEC-0003 契约改 `appreciationLetters.api.ts` —— 写 payload 移除 `senderUserId`/`kindergartenId`/`status`（DTO 仅 `targetType/targetId/title/content/isPublic`，update 仅 `title/content/isPublic`）、解除 `appreciationLettersUnavailable()` 占位接真实端点、VO 改用 `senderName/targetName`（无裸 ID）、重新激活 letters 前端簇。 | P2（现可进行） |
