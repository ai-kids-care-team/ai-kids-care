...
Implementation: `In Progress`（分阶段；下方“分阶段实施边界”的阶段 2、3 已实现并本地验证，阶段 4 部分完成，阶段 5、6 未开始）

> 实施状态（2026-06-15，分支 `codex/spec-0001-auth-context`，未合入 `develop`）：阶段 2 Session Identity Foundation（Redis Spring Session、最小 `SessionPrincipal`、login/session/logout/csrf、cookie、去 JWT）与阶段 3 Context And Default Enforcement（`EffectiveAuthorizationContextFilter`/`Resolver`、每请求权威解析、`@EnableMethodSecurity` + 集中 `AuthorizationPolicy`、平台 tenant-context command、默认 `authenticated`）已实现；阶段 4 Tenant Repository Migration 仅完成 Class/Room/CCTV/CameraStream/DetectionSession/Announcement 的部分 tenant-aware 查询与隐藏 404。本地验证：后端 `gradlew test` 61 通过/0 失败/2 预期 skip；前端 scoped lint 0、生产构建 20 页。详见 [SPEC-0001 实施记录](../../specs/SPEC-0001-auth-authorization-tenant-sensitive-data-boundaries.md#2026-06-15-phase-23会话认证--有效授权上下文与默认拒绝-本地验证通过未合入)。尚未实现：全发布 operation 分类、Guardian/Teacher 关系策略、全会话吊销、安全审计、TLS、生产门禁与 fresh review。
