...
✅ 来源：`backend/.../controller/`（23 个控制器）、`db/initdb/01_create_schema.sql`（28 张表）+ `02_menu.sql`/`03_CommonCode.sql`（`menu`/`common_codes` 字典 2 表）= 共 30 张、前端 `frontend/src/app/` 路由。完整端点见 [api/rest-endpoints.md](../api/rest-endpoints.md)。
...
| 登录 / 刷新 / 登出 | `AuthController` `/auth/login` `/refresh` | `users`,`user_role_assignments` | ✅ 仅接受恰好一条 ACTIVE role；缺失/多条均 401；✅ 服务端会话（ADR-0016，PR #89）；logout/logout-all 已实现 |
...
| 园级注册审批 | `AdminKindergartenController` `/admin/kindergarten` | `users`,`user_role_assignments`；✅ GET registrations；POST approve/reject/disable（SPEC-0002） |
| 平台注册审批（SUPERADMIN） | `AdminPlatformController` `/admin/platform` | `users`,`user_role_assignments`；✅ GET superadmin-registrations；POST approve/reject/disable（SPEC-0002） |
...
| 检测事件 | （无 Controller；`DetectionEventController` 不存在）| `detection_events`；当前无公共 operation，前端路由显示待授权提示；等待 authenticated tenant/resource policy |
...
| 感谢信 | `AppreciationLetterController` | `appreciation_letters`；✅ 全套 CRUD 已开放（GET 列表/详情 + POST + PUT/{id} + DELETE/{id}，BE-5，2026-06-20；sender/tenant 服务端派生，`editable` 字段服务端派生） |
...
| 审计日志 | （无 Controller；`AuditLogController` 不存在）| `audit_logs`；无公共 operation；等待内部 append writer 与授权查询 |
...
- 感谢信：`appreciationLetter`、`letters` 已激活，接后端 BE-5 端点（FE-2，2026-06-20）
