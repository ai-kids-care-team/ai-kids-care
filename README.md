...
- 회원가입, 로그인, 서버 세션 인증(Spring Session + Redis + cookie + CSRF), 역할 기반 메뉴 제공
...
| 영역 | 내용 |
| --- | --- |
| 인증과 권한 | `GUARDIAN`, `TEACHER`, `KINDERGARTEN_ADMIN`, `PLATFORM_IT_ADMIN`, `SUPERADMIN` 역할, 서버 세션 인증(Spring Session + Redis + cookie + CSRF) 로그인/갱신/로그아웃, 역할별 메뉴 |
